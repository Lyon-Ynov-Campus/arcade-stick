export class JoyStick {
    constructor(container, parameters = {}) {
        ...
        if (!this.style.width) { this.style.width = objContainer.clientWidth; }
        if (!this.style.height) { this.style.height = objContainer.clientHeight; }
    ...
    registerEvents() {
        ...
        if ("ontouchstart" in document.documentElement) {
            this.canvas.addEventListener("touchstart", (ev) => { this.onTouchStart(ev) }, false);
            this.canvas.addEventListener("touchmove", (ev) => { this.onTouchMove(ev) }, false);
            this.canvas.addEventListener("touchend", (ev) => { this.onTouchEnd(ev) }, false);
        } else {
            ...
            document.addEventListener("keypress", (ev) => { this.onKeyPress(ev) }, false);
            document.addEventListener("keyup", (ev) => { this.onKeyRelease(ev) }, false);
        }
    }
    ...
    get dimensions() {
        return {
            ...
            externalRadius: (this.canvas.width - ((this.canvas.width / 2) + 10)) / 2 + 30,
    ...
    get command() {
        return {
            x: (100 * ((this.status.position.x - this.center.x) / this.dimensions.maxMoveStick)).toFixed(),
            y: ((100 * ((this.status.position.y - this.center.y) / this.dimensions.maxMoveStick)) * -1).toFixed()
        }
    };
    ...
    drawExternal() {
        ...
        this.context.arc(...Object.values(this.center), this.dimensions.externalRadius, 0, this.circumference, false);
     ...
     */
    drawInternal() {
        const pos = this.status.position, iRad = this.dimensions.internalRadius, maxMove = this.dimensions.maxMoveStick
        if (pos.x < iRad) { this.status.position.x = maxMove }
        if ((pos.x + iRad) > this.canvas.width) { this.status.position.x = this.canvas.width - (maxMove); }
        if (pos.y < iRad) { this.status.position.y = maxMove }
        if ((pos.y + iRad) > this.canvas.height) { this.status.position.y = this.canvas.height - (maxMove); }
        ...
        this.context.beginPath();
        this.context.arc(...Object.values(this.status.position), iRad, 0, this.circumference, false);

        // create radial gradient
        var grd = this.context.createRadialGradient(...Object.values(this.center), 5, ...Object.values(this.center), 200);
        // Light color
